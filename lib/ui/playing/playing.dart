...
class _PlayingPageState extends State<PlayingPage>
  ...
  late LoopMode _loopMode = LoopMode.off; // Trạng thái lặp bài hát.
  ...
  void initState() {
    ...
    // Lắng nghe trạng thái bài hát để tự động chuyển bài khi kết thúc.
    ...
    return SizedBox(
        child: Row(
          ...
          children: [
            ...
            // Nút bật/tắt chế độ lặp lại bài hát.
  ...
  /// Thay đổi chế độ lặp lại bài hát (Tắt -> Lặp một bài -> Lặp tất cả danh sách).
  void _setRepeat() {
    setState(() {
      ...
      // Nếu là lặp một bài, cấu hình trực tiếp trong AudioPlayer.
      // Nếu là lặp tất cả, chúng ta sẽ tự xử lý chuyển bài khi bài hát kết thúc.
  ...
  /// Trả về icon tương ứng với chế độ lặp hiện tại.
  IconData _getRepeatIcon() {
    return _loopMode == LoopMode.one ? Icons.repeat_one : Icons.repeat;
  }

  /// Trả về màu sắc cho nút Repeat dựa trên trạng thái.
  ...
  void _setNextSong() {
    if (_isShuffle) {
      ...
      // Nếu lặp tất cả và đang ở bài cuối, quay về bài đầu tiên.
      _selectedItemIndex = 0;
    } else {
      // Nếu không lặp và đã hết danh sách, dừng lại.
      return;
    }
    ...
    _audioPlayerManager.player.play(); // Tự động phát bài tiếp theo.
  ...
  void _setPreviousSong() {
    if (_isShuffle) {
      ...
      // Nếu lặp tất cả và đang ở bài đầu tiên, quay về bài cuối cùng.
      _selectedItemIndex = widget.songs.length - 1;
    } else {
      // Nếu không lặp và ở bài đầu tiên, có thể quay về đầu bài hát.
    ...
    _audioPlayerManager.player.play(); // Tự động phát bài trước đó.
    _resetRotationAnim(); // Đặt lại hoạt ảnh xoay ảnh bìa.
    setState(() {
